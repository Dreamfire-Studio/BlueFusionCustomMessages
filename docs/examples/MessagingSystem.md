...
* **Template-driven**: Source of truth is `BlueFusionCustomMessagesMessages` (enum). Storage & delivery via `BlueFusionCustomMessagesMessagesConfig`.
...
```
BlueFusionCustomMessagesMessages   (enum of message keys + default templates)
            │
            ▼
BlueFusionCustomMessagesMessagesConfig (StaticEnumPulseConfig<Messages, String>)
...
* **`BlueFusionCustomMessagesMessages`**: Enum of message keys. Each entry holds a template with `%s` placeholder(s). Convention: the *first* `%s` is always the plugin name.
* **`BlueFusionCustomMessagesMessagesConfig`**: Stores/resolves templates and exposes delivery helpers. Responsible for:
...
```java
messagesConfig.SendMessageToBroadcast(BlueFusionCustomMessagesMessages.ConsoleEnabledSystem,
...
```java
messagesConfig.SendMessageToPlayer(BlueFusionCustomMessagesMessages.PlayerReloadedConfig,
...
```java
messagesConfig.SendMessageToConsole(BlueFusionCustomMessagesMessages.SystemIsntEnabled,
    DreamMessageSettings.all());
```
...
```java
messagesConfig.SendMessageToContext(BlueFusionCustomMessagesMessages.PlayerSerializedItem,
...
* Iterates online players and checks perms via `BlueFusionCustomMessagesPermissionsConfigs`.
...
* `SendMessageToPlayerPermission` queries `BlueFusionCustomMessagesPermissionsConfigs` for efficient, consistent checks.
...
* Listeners can broadcast or target messages when reacting to events like `BlueFusionCustomMessagesConfigReloadEvent`.
...
* `BlueFusionCustomMessagesMessagesConfig` persists templates, enabling server operators to modify messages per key.
...
* Ensure all messages exist in `BlueFusionCustomMessagesMessages` to centralize control.
...
* **Enum**: `BlueFusionCustomMessagesMessages`
* **Storage & Send**: `BlueFusionCustomMessagesMessagesConfig`
