...
### 1) `AbstractBlueFusionCustomMessagesEvent`
...
* **Dispatch gating:** `callEvent()` checks the global configuration (`BlueFusionCustomMessagesConfig.systemEnabled`). If disabled, the event is **skipped**.
...
These concrete events extend `AbstractBlueFusionCustomMessagesEvent` and are already emitted by the core at specific times:

### `BlueFusionCustomMessagesConfigReloadEvent`

Fired after configuration has been re-registered (reload). Listeners may safely read the new values.

### `BlueFusionCustomMessagesConfigResetEvent`

Fired after configuration has been reset to defaults. Typically follows a full static re-registration.

### `BlueFusionCustomMessagesSystemToggleEvent`
...
> Emitted by `BlueFusionCustomMessagesConfig.ToggleSystemEnabled(...)` before persistence so listeners can observe the transition moment.
...
```java
public final class RegionRedrawRequestedEvent extends AbstractBlueFusionCustomMessagesEvent {
...
`callEvent()` internally reads `BlueFusionCustomMessagesConfig.systemEnabled`. If false, the event is not fired (and a debug line is emitted when `debugConfig=true`). Publishers **do not** need to duplicate this check.
...
public final class MyListener implements Listener {
  @EventHandler
  public void onReload(BlueFusionCustomMessagesConfigReloadEvent e) {
...
When `BlueFusionCustomMessagesConfig.debugConfig` is `true`:
...
### `AbstractBlueFusionCustomMessagesEvent`
