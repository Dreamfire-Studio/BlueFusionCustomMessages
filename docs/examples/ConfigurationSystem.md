...
1. **Static Configs**
    ...
    * Example: `BlueFusionCustomMessagesConfig` handles system enable/disable state and debug toggles.
...
2. **Enum-Based Configs**
    ...
    * Example: `BlueFusionCustomMessagesInventoryItemsConfig` binds enum values like `BlankTile` or `SystemEnabled` to concrete `ItemStack` definitions.

3. **Feature Flags**

    * Enum-driven toggles for enabling/disabling optional systems.
    * Defined in `BlueFusionCustomMessagesFeatureFlagsConfig` with default values.
